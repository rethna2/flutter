...
class _CoolSwiperState extends State<CoolSwiper>
  ...
  void playaudio() async {
    if (widget.audio != 'none') {
      var start, end;
      if (widget.audioOffsets != null) {
        start = index == 0 ? 0 : widget.audioOffsets?[index - 1];
        end = widget.audioOffsets?[index];
      } else {
        start = widget.audioOffset + (index) * widget.audioWidth;
        end = widget.audioOffset +
    ...
    return Stack(
      children: [
        Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text(
              widget.title ??
                  'Move the cards and remember the picture, sound and the word.',
...
class CardContent extends StatelessWidget {
  final Color color;
  final List<Widget> children;
  const CardContent({Key? key, required this.color, required this.children})
      : super(key: key);
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Align(
        ...
        child: Container(
            width: Math.min(double.infinity, 400),
...
class Constants {
  ...
  static const double yOffset = 11; // 13 -
